...
| OIDC_REDIRECT_URI          | The URL to which the OIDC provider should redirect the user after authentication. This URL must be registered as a redirect URI in the OIDC provider (e.g., `http://localhost:3000/callback` or `/callback`).                                                                                                                   | `/callback`                                                                                                                                                                                                                               |
...
If you are using HTTP instead of HTTPS during local development, make sure you specify `OIDC_AUTH_EXTERNAL_URL` and `OIDC_REDIRECT_URI` to include `http` in front of `localhost` to avoid hiccups.
