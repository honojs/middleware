...
import type { FacebookErrorResponse, FacebookTokenResponse, FacebookUser } from '../src/providers/facebook'
import type { GithubErrorResponse, GithubTokenResponse } from '../src/providers/github'
import type { GoogleErrorResponse, GoogleTokenResponse, GoogleUser } from '../src/providers/google/types'
import type { LinkedInErrorResponse, LinkedInTokenResponse } from '../src/providers/linkedIn'

export const handlers = [
  // Google
  http.post('https://oauth2.googleapis.com/token', async ({ request }): Promise<StrictResponse<Partial<GoogleTokenResponse> | GoogleErrorResponse>> => {
  ...
  http.get('https://www.googleapis.com/oauth2/v2/userinfo', async ({ request }): Promise<StrictResponse<Partial<GoogleUser> | GoogleErrorResponse>> => {
  ...
  // Facebook
  http.get('https://graph.facebook.com/v18.0/oauth/access_token', async ({ request }): Promise<StrictResponse<Partial<FacebookTokenResponse> | FacebookErrorResponse>> => {
  ...
  http.get('https://graph.facebook.com/1abc345-75uyut', async ({ request }): Promise<StrictResponse<Partial<FacebookUser> | FacebookErrorResponse>> => {
  ...
  // Github
  http.post('https://github.com/login/oauth/access_token', async ({ request }): Promise<StrictResponse<Partial<GithubTokenResponse | GithubErrorResponse>>> => {
    const body = await request.json() as Promise<DefaultBodyType> & { code: string }
    if (body.code === dummyCode) {
  ...
  // LinkedIn
  http.post('https://www.linkedin.com/oauth/v2/accessToken', async ({ request }): Promise<StrictResponse<Partial<LinkedInTokenResponse> | LinkedInErrorResponse>> => {
    const code = new URLSearchParams(request.url).get('code')
    if (code === dummyCode) {
