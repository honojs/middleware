 ...
 */
export type RefreshSession<Data> = (expired: Data) => Promise<Data | null>

type Set<Data> = Data | ((prevData: Data | null) => Data)
...
export interface Session<Data> {
   ...
   */
  get: (refresh?: RefreshSession<Data>) => Promise<Data | null>

  /**
   * Update the current session with the provided session data.
   */
  update: (data: Set<Data>) => Promise<void>
}
...
export function getSession<Data extends SessionData>({
  ...
  async function persistSession(): Promise<void> {
    if (session?.action === 'update') {
      ...
      if (!storage || session.payload.exp === undefined) {
        ...
        cookie.set(cookie.name, jwe, { ...cookie.options, maxAge })
      }
    }
    ...
    if (session?.action === 'destroy') {
      ...
      cookie.delete(cookie.name, cookie.options)
    }
  }
  ...
  async function getSessionCookie(): Promise<cookies.DecryptResult<SessionCookie> | undefined> {
    const value = cookie.get(cookie.name, cookie.options?.prefix)
    let result
